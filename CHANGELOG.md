...
| v0.11 (this round) | 2026-05-31 | tbd | (Crinecerfont alone: 1/1 vs 0/1 in v0.9.4) | tbd | g2p-rag v0.1.1 chunk enrichment lands. Re-ingested 47 benchmark genes; ChromaDB grew 684 -> 819 chunks (+135 biology chunks: function=48, subunit=43, disease=40, pathway=4). Crinecerfont single-case retest: predicted CRHR1, target_recovered=true (was MC2R = false in v0.9.4). Confidence 0.75 (above min 0.65). The trajectory CYP21A2 -> NR3C1 -> MC2R -> **CRHR1** is the full closure of the feedback-axis failure mode that the v0.9.x guards opened. **(See "v0.11 audit follow-up" below — this recovery is partly prompt-leakage, not a clean g2p-rag win.)** |
| v0.11 audit (2026-06-01) | 2026-06-01 | -- | (Crinecerfont CRHR1 walkback) | -- | Post-hoc audit found that the v0.11 CRHR1 recovery is partly attributable to prompt-level hints in `therapy-agent/src/therapy_agent/nodes/strategy_synthesis.py`. Both Stage-1 `_PATTERN_SELECTOR_SYSTEM` (lines 82, 92) and Stage-2 `_TARGET_PICKER_SYSTEM` (line 141) explicitly name CRHR1 as the right answer for the CYP21A2 + ACTH-driven CAH archetype. The Crinecerfont test case is a substring match for the negative example on line 92. Since CRHR1 is not a Reactome / UniProt pathway interactor of CYP21A2, it is not in the g2p-rag candidate list shown to the Stage-2 picker -- the model selects CRHR1 because the prompt tells it to, not because retrieval surfaced it. The "first measurable g2p-rag improvement" claim is walked back to "partly measurable, partly prompt-injected": the chunk count and rationale text are enriched, but the target-symbol choice cannot be attributed to g2p-rag alone. See [ROUND_NOTES_v0.11.md](ROUND_NOTES_v0.11.md) "v0.11 audit follow-up" for full leak-site quotes. |
