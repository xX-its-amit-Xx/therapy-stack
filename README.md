...
**v0.11 (this round): partly measurable g2p-rag improvement — walked back.** Re-ingested all 47 benchmark genes with the v0.1.1 enriched chunker (`function` / `pathway` / `subunit` / `disease` chunk types added; +135 chunks vs prior). Crinecerfont single-case re-test predicted **CRHR1** (target_recovered=true, confidence 0.75). Trajectory across versions: CYP21A2 (v0.9 baseline) → NR3C1 (v0.9.2b) → MC2R (v0.9.4) → **CRHR1** (v0.11). **Audit follow-up (2026-06-01):** a post-hoc review found this CRHR1 recovery is partly attributable to *prompt-level hints*, not purely g2p-rag retrieval. Both Stage-1 `_PATTERN_SELECTOR_SYSTEM` (lines 82, 92) and Stage-2 `_TARGET_PICKER_SYSTEM` (line 141) in `therapy-agent/src/therapy_agent/nodes/strategy_synthesis.py` explicitly name **CRHR1** as the correct answer for the CYP21A2 + ACTH-driven CAH archetype — the Crinecerfont test case is a substring match for the negative example on line 92. Since CRHR1 is not a Reactome / UniProt pathway interactor of CYP21A2, it is not in the g2p-rag candidate list shown to the picker; the model is naming CRHR1 because the prompt told it to. The original "first measurable g2p-rag improvement" framing is walked back to **"partly measurable, partly prompt-injected"**: the new chunks do enrich the rationale text with HPA-axis language, but the target-symbol choice cannot be attributed to g2p-rag alone. A clean test requires a feedback-axis case whose specific answer is *not* named in the prompts. See [`ROUND_NOTES_v0.11.md`](ROUND_NOTES_v0.11.md) "v0.11 audit follow-up" for the full leak-site quotes and the [`CHANGELOG.md`](CHANGELOG.md) `v0.11 audit` row.
